"""Visual reading-order repair for reverse-paint PDF text streams.

Design PDFs often paint decorative titles (``WHO HAS ORGASMS?``) as
per-character objects in reverse paint order.  Stream unicode then
becomes ``?SMSrgao SahWho``.  This module detects reverse-dominant lines
and reorders characters top-to-bottom, left-to-right before unicode
assembly and MT.

Owns only geometry/order; callers decide when to apply.
"""

from __future__ import annotations

from babeldoc.format.pdf.document_il.il_version_1 import Box
from babeldoc.format.pdf.document_il.il_version_1 import PdfCharacter

# Same-line reverse pairs / total pairs ≥ this → consider reverse-paint.
# OA p7/8: body ≈ 0.0, decorative titles ≈ 0.25–0.57.
_DEFAULT_REVERSE_RATIO = 0.20
_DEFAULT_Y_TOL = 6.0
_DEFAULT_MIN_PAIRS = 4
# Guard: only reorder short decorative-like runs, not long body lines that
# happen to have a few RTL pairs (zigzag / multi-column noise).
_MAX_REORDER_CHARS = 64
_MIN_SINGLE_LETTER_RATIO = 0.55


def _resolve_char_box(char: PdfCharacter) -> Box | None:
    """Prefer visual_bbox when it overlaps pdf box (same policy as
    ``ParagraphFinder._get_char_sort_key``); else fall back to either.
    """
    visual = None
    vb = getattr(char, "visual_bbox", None)
    if vb is not None and getattr(vb, "box", None) is not None:
        visual = vb.box
    pdf = getattr(char, "box", None)

    if visual is not None and pdf is not None:
        try:
            from babeldoc.format.pdf.document_il.utils.layout_helper import (
                calculate_iou_for_boxes,
            )

            if calculate_iou_for_boxes(visual, pdf) >= 0.1:
                return visual
            return pdf
        except Exception:
            return visual
    return visual if visual is not None else pdf


def char_visual_xy(char: PdfCharacter) -> tuple[float, float] | None:
    """Return ``(x, y_mid)`` for reading-order comparisons."""
    box = _resolve_char_box(char)
    if box is None or box.x is None or box.y is None:
        return None
    y2 = box.y2 if box.y2 is not None else box.y
    return (float(box.x), (float(box.y) + float(y2)) / 2.0)


def is_stream_visually_reversed(
    chars: list[PdfCharacter],
    *,
    reverse_ratio: float = _DEFAULT_REVERSE_RATIO,
    y_tol: float = _DEFAULT_Y_TOL,
    min_pairs: int = _DEFAULT_MIN_PAIRS,
) -> bool:
    """True when same-line stream order is mostly right-to-left."""
    pdf_chars = [c for c in chars if isinstance(c, PdfCharacter)]
    if len(pdf_chars) < min_pairs + 1:
        return False

    decreasing = 0
    total = 0
    for i in range(len(pdf_chars) - 1):
        a = char_visual_xy(pdf_chars[i])
        b = char_visual_xy(pdf_chars[i + 1])
        if a is None or b is None:
            continue
        if abs(a[1] - b[1]) > y_tol:
            continue  # different visual line — not an RTL pair
        total += 1
        if b[0] < a[0] - 0.5:
            decreasing += 1
    if total < min_pairs:
        return False
    return (decreasing / total) >= reverse_ratio


def _looks_like_decorative_run(chars: list[PdfCharacter]) -> bool:
    """True for short, mostly single-letter runs (typical reverse-paint titles).

    Long body lines almost never reverse-paint as a whole; requiring this
    keeps reverse_ratio=0.20 from reshuffling multi-column body noise.
    """
    pdf_chars = [c for c in chars if isinstance(c, PdfCharacter)]
    if not pdf_chars or len(pdf_chars) > _MAX_REORDER_CHARS:
        return False
    letters = 0
    single = 0
    for c in pdf_chars:
        u = (c.char_unicode or "").strip()
        if not u or u.isspace():
            continue
        letters += 1
        if len(u) == 1 and u.isalpha():
            single += 1
    if letters < 3:
        return False
    return (single / letters) >= _MIN_SINGLE_LETTER_RATIO


def sort_chars_visual_order(
    chars: list[PdfCharacter],
    *,
    y_tol: float = _DEFAULT_Y_TOL,
) -> list[PdfCharacter]:
    """Sort characters top-to-bottom, left-to-right within each visual line.

    Algorithm:
      1. Cluster by baseline y (bucket width = y_tol).
      2. Within each cluster, sort by x ascending (LTR).
      3. Emit clusters top-to-bottom (PDF y grows up → higher y first).
    """
    if len(chars) < 2:
        return list(chars)

    # (orig_idx, y_mid, x, char)
    items: list[tuple[int, float, float, PdfCharacter]] = []
    for i, ch in enumerate(chars):
        xy = char_visual_xy(ch)
        if xy is None:
            items.append((i, 0.0, float(i), ch))
        else:
            items.append((i, xy[1], xy[0], ch))

    # Cluster by y first (bucket), preserve top-to-bottom via negated bucket
    buckets: dict[int, list[tuple[int, float, float, PdfCharacter]]] = {}
    for item in items:
        bucket = int(round(item[1] / y_tol))
        buckets.setdefault(bucket, []).append(item)

    ordered: list[PdfCharacter] = []
    # Higher PDF y = higher on page → emit larger buckets first
    for bucket in sorted(buckets.keys(), reverse=True):
        line = buckets[bucket]
        line.sort(key=lambda t: (t[2], t[0]))  # x, then stable orig index
        ordered.extend(t[3] for t in line)
    return ordered


def maybe_reorder_reversed_stream(
    chars: list[PdfCharacter],
) -> list[PdfCharacter]:
    """If stream is reverse-dominant decorative text, return visual LTR order.

    Otherwise returns the same list object (no copy) so callers can detect
    identity and skip ``update_line_data``.
    """
    if not chars:
        return chars
    if not _looks_like_decorative_run(chars):
        return chars
    if not is_stream_visually_reversed(chars):
        return chars
    return sort_chars_visual_order(chars)
