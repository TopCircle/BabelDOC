...
def _char_visual_xy(char: PdfCharacter) -> tuple[float, float] | None:
    """Return (x, y_mid) from visual_bbox, falling back to box."""
    box = None
    ...
    if box is None or box.x is None or box.y is None:
        return None
    y2 = box.y2 if box.y2 is not None else box.y
    return (float(box.x), (float(box.y) + float(y2)) / 2.0)


def is_stream_visually_reversed(
    chars: list[PdfCharacter],
    *,
    reverse_ratio: float = 0.20,
    y_tol: float = 6.0,
    min_pairs: int = 4,
) -> bool:
    """True when same-line stream order is mostly right-to-left.

    Design PDFs often paint decorative titles (``WHO HAS ORGASMS?``,
    ``PREPARE FOR THE BEST``) as per-character objects in reverse paint
    order.  Stream unicode then becomes ``?SMSrgao SahWho``.  Body prose
    on the same pages has reverse ratios near 0.0, so a low threshold is
    safe (see Orgasmic Addiction p7/8 IL dumps).
    """
    ...
    for i in range(len(pdf_chars) - 1):
        a = _char_visual_xy(pdf_chars[i])
        b = _char_visual_xy(pdf_chars[i + 1])
        if a is None or b is None:
            continue
        if abs(a[1] - b[1]) > y_tol:
            continue  # different visual line — not an RTL pair
        total += 1
        if b[0] < a[0] - 0.5:
            decreasing += 1
    if total < min_pairs:
...
def sort_chars_visual_order(
    ...
    *,
    ...
    Used to repair reverse-paint decorative titles before unicode assembly
    and MT.  Stable for already-correct LTR prose (no-op on ordering).
    """
    if len(chars) < 2:
        return list(chars)

    indexed: list[tuple[int, float, float, PdfCharacter]] = []
    for i, ch in enumerate(chars):
        xy = _char_visual_xy(ch)
        if xy is None:
            indexed.append((i, 0.0, float(i), ch))
        else:
            # PDF y grows upward → negate for top-to-bottom
            indexed.append((i, -xy[1], xy[0], ch))

    # Cluster into lines by y (already using -y in sort key primary)
    indexed.sort(key=lambda t: (round(t[1] / y_tol), t[2], t[0]))
...
def maybe_reorder_reversed_stream(
    ...
    """If stream is reverse-dominant, return visual LTR order; else unchanged."""
    if not chars or not is_stream_visually_reversed(chars):
        return chars
