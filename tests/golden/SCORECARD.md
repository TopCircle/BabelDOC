...
| Trailing serial reattach (ATU p21 items 4/5) | ✅ 0.6.4.8: always **strip** trailing ``。4.`` even when next already has leading serial; normalize ``N。``→``N.`` |
...
| PDF size / CJK font subset | ✅ 0.6.4.12: always subset; operator accepted. **0.6.4.13:** subset **only** BabelDOC embedding fonts — protect original faces for skipped pages / header-footer / figure text |
